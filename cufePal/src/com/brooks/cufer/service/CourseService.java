...
public class CourseService
{
		...
		for (int i = 0; i < rowNum - 1; i++)
		{
			...
			for (int j = 1; j < columnNum; j++)
			{
				...
				if (column.hasAttr("rowspan"))
				{
					try
					{
						splitCourse(column.html(), startYear, endYear,
								semester, week, i + 1,
			...
			int semester, int week, int startSection, int endSection)
	{
		...
		// 运筹学<br />必修<br />周三第1,2节{第1-17周|单周}<br />冯海旗<br />沙河主教406M
		String reg = "周?(.)?第?(\\d{1,2})?,?(\\d{1,2})?,?(\\d{1,2})?节?\\{第(\\d{1,2})-(\\d{1,2})周\\|?((.*周))?\\}";
		...
		System.out.println("是否陪陪---->" + matcher.matches());
		// 周几，可能为空，此时使用传进来的值
	 ...
	 * @return
	 */
	private int splitCourse(String str, int startYear, int endYear,
			int semester, int week, int startSection, int endSection)
	{
		...
		{// 如果大于一节课
			...
			{
				if (!(split[i].startsWith("<br />") && split[i]
						.endsWith("<br />")))
				{
					storeCourseByResult(split[i], startYear, endYear, semester,
							week, startSection, endSection);// 保存单节课
				} else
				{
					...
					storeCourseByResult(substring, startYear, endYear,
							semester, week, startSection, endSection);// 保存单节课
				}
			}
			return split.length;
		} else
		{
			storeCourseByResult(str, startYear, endYear, semester, week,
					startSection, endSection);// 保存
			return 1;
		}
	}
	/**
